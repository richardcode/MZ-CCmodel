...
c     circulation of carbon.
	  ...
	  integer nstop,n,k,j,kmin,n1,n2,n3,ndump, num_args
...
c            pco2(n)=28.35
c          elseif (n.gt.n1.and.n.lt.n2) then
cc            pco2(n)=((n*dt-t1)/(t2-t1))
cc            pco2(n)=28.35+(pco20-28.35)*(sin(0.5*pi*pco2(n)))**2
cc            Increase partial pressure of CO2 by 1%/yr to 4xCO2 stabilisation
c             pco2(n)=28.35*exp(log(1.01)*(n*dt-t1)/year)
c          elseif (n.ge.n2) then
c            pco2(n)=28.35*exp(log(1.01)*(n2*dt-t1)/year)
c          endif
c        enddo
c      print *, pco2


!      pCO2 load from a file
       call get_command_argument(1,length=arglen)
       allocate(character(arglen) :: fname)
